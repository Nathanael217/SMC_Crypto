...
def _render_view_block(
    ...
    When view_data is None we still render an explicit placeholder so the
    user can see that this view was simply NOT DETECTED (no nested LH-LL
    pair found inside the broad leg, or no broad pair at all) — rather
    than silently hiding the section and leaving the user wondering
    whether it's a bug or missing data.
    """
    ...
    if view_data is None:
        # Explicit "not detected" placeholder — same shape as a real block
        # so layout doesn't shift between coins that have / don't have it.
        if view_label.lower() == "narrow":
            st.info(
                "🔍 No nested NARROW LH-LL pair detected inside the broad leg. "
                "Normal for one-clean-leg downtrends — no secondary structure "
                "formed after the broad LL. Trade BROAD view only."
            )
        else:
            st.info(
                "🔭 No BROAD LH-LL pair detected. Structure may be too fresh "
...
def render_signal_card_short(
    ...
    with st.expander(
        ...
        # ── Dual-View: BROAD + NARROW (always show both — see helper) ────────
        # Even when one view is None we still call _render_view_block so the
        # user sees an explicit "not detected" placeholder rather than a
        # silently hidden section. Layout stays consistent across coins.
