...
def _render_view_block_long(
    ...
    Mirrors the SHORT scanner's _render_view_block: when view_data is None
    we still render an explicit placeholder so the user can see that this
    view was simply NOT DETECTED — rather than silently hiding the section.
    """
    active_tag = " 🔵 **ACTIVE**" if is_active else ""
    st.markdown(f"**{label} VIEW**{active_tag}")

    if view_data is None:
        # Explicit "not detected" placeholder — keeps layout consistent
        # across coins that have / don't have this view.
        if "NARROW" in label.upper():
            st.info(
                "🔍 No nested NARROW HL-HH pair detected inside the broad leg. "
                "Normal for one-clean-leg uptrends — no secondary structure "
                "formed after the broad HH. Trade BROAD view only."
            )
        else:
            st.info(
                "🔭 No BROAD HL-HH pair detected. Structure may be too fresh "
                "or only NARROW pair available."
            )
        return
    ...
    st.markdown(f"{emoji} `{ret_status}`")
    ...
    if smart_obs:
        ...
        st.text("  OBs: none in this view's 0.786 zone")
    # FVGs
    if fvgs_list:
        ...
        st.text("  FVGs: none in this view's 0.786 zone")
    # S/R
    if srs:
...
def render_signal_card_detail_v2(
    ...
    # Mirrored from SHORT render: when view_used is None (APPROACHING-only
    # case → setup not yet tradeable) we show an explicit waiting badge so
    # the user knows why the card isn't in a zone bucket.
    if view_used == "BROAD":
        view_badge = "[🔭 BROAD]"
    ...
    with st.expander(
        ...
        with col1:
            ...
            # ── Dual-view blocks (LONG hierarchical views; always show both) ─
            # Even when one view is None we still call _render_view_block_long
            # so the user sees an explicit "not detected" placeholder rather
            # than a silently hidden section. Mirrors SHORT render behaviour.
