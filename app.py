...
def render_smc_short_mode_scan_subtab():
    ...
    with st.expander("⚙️ Advanced Settings", expanded=False):
        ...
        # NEW: min bounce filter (mirror LONG tab — Session 5 parity)
        min_bounce_choice_short = st.radio(
            "Minimum bounce filter (for LH validity)",
            options=["0.236 (looser, more setups)", "0.382 (stricter, cleaner setups)"],
            index=0,
            key="smc_short_min_bounce",
            help=(
                "Minimum % bounce from prior LL→LH to qualify as a structural "
                "down-leg. 0.236 captures more setups; 0.382 is stricter. "
                "Mirrors the LONG scanner's min-bounce filter."
            ),
        )
        min_bounce_pct_short = 0.236 if min_bounce_choice_short.startswith("0.236") else 0.382
        st.session_state["smc_short_min_bounce_pct"] = min_bounce_pct_short
    ...
    if st.button("🚀 Run SMC Short Scan", type="primary", key="smc_short_run_scan"):
        ...
        with st.spinner(spinner_msg):
            results = run_scan_short(
                ...
                run_backtest=run_bt,
                min_bounce_pct=st.session_state.get("smc_short_min_bounce_pct", 0.236),  # NEW: mirror LONG
                progress_callback=progress_callback,
            )
